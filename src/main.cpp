...
void CryptobullionMiner(CWallet *pwallet, bool fProofOfStake)
{
    ...
    while (fGenerateCryptobullions || fProofOfStake)
    {
        if (fShutdown)
            return;
        while (vNodes.empty() || IsInitialBlockDownload())
        {
            ...
            if (fShutdown)
                return;
            if ((!fGenerateCryptobullions) && !fProofOfStake)
                return;
        }
        ...
        if (fProofOfStake)
        {
            ...
            if (pblock->IsProofOfStake())
            {
                ...
                strMintWarning = "";
                printf("CPUMiner : PoSP block found %s\n", pblock->GetHash().ToString().c_str());
        ...
        printf("Running CryptobullionMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
