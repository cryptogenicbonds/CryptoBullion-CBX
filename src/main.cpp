...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        ...
        int64 nCalculatedStakeReward = GetProofOfStakeReward(nCoinAge, (pindex != NULL && pindex->pprev != NULL) ? pindex->pprev->nBits : 0, nTime, (pindex != NULL && pindex->pprev != NULL) ? pindex->pprev->nHeight : ((pindex != NULL && pindex->pprev == NULL) ? 1 : 0), (pindex != NULL && pindex->pprev != NULL) ? pindex->pprev->nMoneySupply : 0, nFees);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    else if (strCommand == "inv")
    {
        ...
        for (unsigned int nInv = 0; nInv < vInv.size(); nInv++)
        {
            ...
            if (!fAlreadyHave)
                pfrom->AskFor(inv, IsInitialBlockDownload()); // Imidiate retry when initial download
